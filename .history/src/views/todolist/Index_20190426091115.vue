<template lang="pug">
  .main-container(v-cloak)
    ...
    div
      ...
      Todo(v-for='(todo, index) in todosFiltered', :key='todo.id', :todo="todo" :index='index' :bind = 'a')
      {{a}}
      .extra-container
  ...
  methods: {
    ...
    addNewTodo() {
      ...
      .then(() => {
        ...
        // localStorage.setItem('todos', JSON.stringify(this.getTodos)) // при успехе тоже  добавление
      })
      .catch(error => {
        console.log('Ошибка при запросе на Ваш сервер')
        this.getTodos.push(data)
        // localStorage.setItem('todos', JSON.stringify(this.getTodos)) // при ошибке только добавление
      })
      this.firstTodo = ''
      this.idForTodo++
    },

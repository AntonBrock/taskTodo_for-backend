...
export default {
  ...
  //осталась одна логика. Если при перезагрузки в API пусто, то нужно вернут статику - а не пустой todo, 
  // но если в дэмо режиме, то обновление должно выдавать список наш
  created(){
    this.a = JSON.parse(localStorage.getItem('todos'))
      
      if(this.updateTodoList([])) {

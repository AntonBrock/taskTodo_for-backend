<template lang="pug">
  .todo-item
    .todo-item-left
      ...
      div.todo-item-label(v-bind:class='{ completed : todo.completed }') {{ todo.title }}
    .remove-item
      label.auto_created Авто-создание
      input(type='checkbox', :disabled="todo.completed", :checked="todo.autocreate" @click='checkIt(todo)') 
      i.fas.fa-times.remove(@click='removeItem(todo.id, index)')
</template>
...
export default {
  ...
  methods: {
    ...
    removeItem(id, index) {
      ...
      if (this.getUrl.length > 3) {
        todoApi
          .deleteTodo(id) // тут id а не index
          .then(() => {
            console.info("Удачно");

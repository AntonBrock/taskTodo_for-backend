<template lang="pug">
  .todo-item
    .todo-item-left
      ...
      div.todo-item-label(v-on:dblclick="changeTodo(todo)" v-bind:class='{ completed : todo.completed }') {{ todo.title }}
      input.todo-item-edit(v-else type="text" v-model="todo.title" @blur="doneEdit(todo)" @keyup.enter="
      (todo)" @keyup.esc="cancelEdit(todo)")
    .remove-item
...
export default {
  ...
  methods: {
    ...
    doneEdit(todo) {
      ...
      if (this.getUrl.length > 3) {
        //тут запрос, на сколько верен не знаю. Он взят с авто-создания, сомнения, что отправляет autocreate, хотя такой же patch
      todoApi
        .editTodo(this.todo.id)
        .then(() => {
          console.info("Удачно");

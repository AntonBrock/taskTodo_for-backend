...
export default {
  ...
  methods: {
    ...
    checkIt(todo) {
      ...
      localStorage.setItem('todos', JSON.stringify(this.getTodos)) // сохраняеет изменения в Ls, но не отображает их! 
    },
    removeItem(index) {
      ...
      this.getTodos.splice(this.getTodos(index), 1)
      localStorage.setItem('todos', JSON.stringify(this.getTodos)) // зачитает с 2 только

      // this.getTodos.splice(this.getTodos.indexOf(index), 1)
